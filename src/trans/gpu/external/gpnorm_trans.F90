...
IF(IUBOUND(1) < NPROMA) CALL ABORT_TRANS('GPNORM_TRANS:FIRST DIMENSION OF PGP TOO SMALL ')
IF(IUBOUND(2) < KFIELDS) CALL ABORT_TRANS('GPNORM_TRANS:SECOND DIMENSION OF PGP TOO SMALL ')
IF(IUBOUND(3) < NGPBLKS) CALL ABORT_TRANS('GPNORM_TRANS:THIRD DIMENSION OF PGP TOO SMALL ')
...
CALL GSTATS(1429,0)
IF( IF_FS > 0 )THEN
  !$ACC DATA COPY(F,F%RW,D,D_NSTAGTF,D_NPTRLS,G_NLOEN,G_NLOEN_MAX) &
  ...
  !$ACC update host(ZAVE,ZMINGPN,ZMAXGPN)
  !$ACC wait
ENDIF
CALL GSTATS(1429,1)
