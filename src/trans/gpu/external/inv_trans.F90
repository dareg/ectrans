...
USE TPM_TRANS       ,ONLY : LDIVGP, LSCDERS, LUVDER, LVORGP, LATLON,  &
     &                      NF_SC2, NF_SC3A, NF_SC3B, NGPBLKS, NPROMA,FOUBUF
...
IF(PRESENT(LDVORGP)) LVORGP = LDVORGP
IF(PRESENT(LDDIVGP)) LDIVGP = LDDIVGP
IF(PRESENT(LDUVDER)) LUVDER = LDUVDER
IF(PRESENT(LDLATLON)) LATLON = LDLATLON

! Compute derived variables

IF (LVORGP.and..NOT.LDIVGP) THEN
  write(nout,*) "LVORGP=T => LDIVGP reset to true"
  LDIVGP = .TRUE.
ENDIF
...
IF(PRESENT(PGPUV)) THEN
  IF(.NOT.PRESENT(PSPVOR)) CALL ABORT_TRANS('PSPVOR HAS TO BE PRESENT WHEN PGPUV IS')
...
ENDIF
IF(PRESENT(PGP2)) THEN
  IF(.NOT.PRESENT(PSPSC2)) CALL ABORT_TRANS('PSPSC2 HAS TO BE PRESENT WHEN PGP2 IS')
ENDIF
IF(IF_SC2_G > 0) THEN
  IF(PRESENT(PGP2)) THEN
...
IF(NPROMATR > 0 .AND. IF_GPB > NPROMATR) THEN
  CALL INV_TRANS_CTL(IF_UV_G,IF_SCALARS_G,IF_GP,IF_FS,IF_OUT_LT,IF_UV,IF_SCALARS,IF_SCDERS,&
...
ELSE
  ...
  CALL LTINV_CTL(IF_OUT_LT,IF_UV,IF_SCALARS,IF_SCDERS,PSPVOR,PSPDIV,PSPSCALAR,&
   &PSPSC3A,PSPSC3B,PSPSC2,FSPGL_PROC=FSPGL_PROC)

  ! from FOUBUF to PGPXXX
  CALL FTINV_CTL(IF_UV_G,IF_SCALARS_G,IF_UV,IF_SCALARS,IF_SCDERS,IF_GP,IF_FS,IF_OUT_LT,&
