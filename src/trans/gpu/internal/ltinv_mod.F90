  ...
  IF(KF_SCALARS > 0)THEN
    ...
    ELSE
      IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
        ...
        CALL PRFI1A(ZIA(IFIRST:ILAST,:,:),PSPSC2,NF_SC2,IDIM2)
        !$ACC END DATA
      ENDIF
      IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
        ...
        DO J3=1,IDIM3
          ...
          CALL PRFI1A(ZIA(IFIRST:ILAST,:,:),PSPSC3A(:,:,J3),IDIM1,IDIM2)
        ENDDO
        !$ACC END DATA
      ENDIF
      IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
        ...
        DO J3=1,IDIM3
          ...
          CALL PRFI1A(ZIA(IFIRST:ILAST,:,:),PSPSC3B(:,:,J3),IDIM1,IDIM2)
        ENDDO
        !$ACC END DATA
      ENDIF
    ENDIF
...
SUBROUTINE PRFI1A(PIA,PSPEC,KFIELDS,KDIM)
  ...
  INTEGER(KIND=JPIM) :: KM,KMLOC,INM, J, JFLD, ILCM

  !$ACC DATA PRESENT(D_NUMP,R_NSMAX,D_MYMS,D_NASM0,PIA,PSPEC)

  !$ACC PARALLEL LOOP COLLAPSE(3) PRIVATE(KM,ILCM,INM)
  DO KMLOC=1,D_NUMP
     DO J=1,R_NSMAX+1
        DO JFLD=1,KFIELDS
           KM = D_MYMS(KMLOC)
