...
INTEGER(KIND=JPIM) :: IST,JGL,IGL,JF_FS,jf,inf
...
CALL GSTATS(430,1)
if (nprintlev > 0)&
  write(nout,*) "transpose to lat. array (CUDAAWARE)",KF_FS,"=",KF_UV_G,KF_SCALARS_G,KF_GP
...
!$ACC END DATA
#else
if (nprintlev > 0) write(nout,*) "transpose to lat.  array",KF_FS,KF_GP,KF_SCALARS_G
...
IF(KF_FS>0) THEN
  INF=size(zgtf,1)
  write(nout,*) "Direct Fourier transforms fs/inf/i1/i2/ii:",kf_fs,inf,ibeg,iend,iinc
  CALL FTDIR(INF)
ENDIF

write(nout,*) "copy Fourier buffer to FOUBUF_IN",kf_fs
