...
MODULE UPDSP_MOD
CONTAINS
SUBROUTINE UPDSP(KF_UV,KF_SCALARS,POA1,POA2,PSPVOR,PSPDIV,PSPSCALAR,&
...
INTEGER(KIND=JPIM) :: IVORS, IVORE, IDIVS, IDIVE, IST ,IEND,J3
...
IF (KF_SCALARS > 0) THEN
  ...
  ELSE
    IF (PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
      IEND  = IST+2*NF_SC2-1
      CALL UPDSPB(NF_SC2,POA1(IST:IEND,:,:),PSPSC2)
      IST=IST+2*NF_SC2
    ENDIF
    ...
    IF (PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
      DO J3=1,UBOUND(PSPSC3A,3)
        ...
        CALL UPDSPB(NF_SC3A,POA1(IST:IEND,:,:),PSPSC3A(:,:,J3))
        IST=IST+2*NF_SC3A
      ENDDO
    ENDIF
    ...
    IF (PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
      DO J3=1,UBOUND(PSPSC3B,3)
        ...
        CALL UPDSPB(NF_SC3B,POA1(IST:IEND,:,:),PSPSC3B(:,:,J3))
        IST=IST+2*NF_SC3B
      ENDDO
    ENDIF
  ENDIF
